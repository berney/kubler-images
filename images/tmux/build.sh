...
#     kubler.sh build -i berney/tmux
# ..and then search the Portage database:
#     eix <search-string>
_packages="app-misc/tmux"
...
configure_builder()
{
    ...
    update_use "$_packages" '+static'
    update_use 'dev-libs/libevent' '+static-libs'
...
finish_rootfs_build()
{
    ...
    #export DISTRIBUTION_DIR="${GOPATH}/src/github.com/berney/tmux"
    #mkdir -p "${DISTRIBUTION_DIR}"

    #git clone https://github.com/berney/tmux.git "${DISTRIBUTION_DIR}"
    #cd "${DISTRIBUTION_DIR}"
    #git checkout tags/v${_tmux_version}
    #echo "building tmux ${_tmux_version}.."
    ...
    #log_as_installed "manual install" "tmux-${_tmux_version}" "https://tmux.org/"
