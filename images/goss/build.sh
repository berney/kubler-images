...
#     kubler.sh build -i kubler-images/goss
...
configure_builder()
{
    ...
    cd goss || exit 1
    # The Makefile calls `release-build.sh` which sets the version string based off the `TRAVIS_TAG`
    # Which, presumably, is the git tag.
    # It defaults to `0.0.0`. I'd rather have the version.
    # `git describe` without `--all` will be something like `v0.4.4-4-g4e36e8f`
    #                with `--all` it will be something like `heads/master-0-g4e36e8f`
    TRAVIS_TAG=$(git describe --long --dirty) make release/goss-linux-amd64
}
...
configure_rootfs_build()
{
    ...
    #init_docs "kubler-images/goss"
    rsync -av /goss/release/goss-linux-amd64 "${_EMERGE_ROOT}"/goss
}
...
finish_rootfs_build()
{
    ...
    #export DISTRIBUTION_DIR="${GOPATH}/src/github.com/kubler-images/goss"
    #mkdir -p "${DISTRIBUTION_DIR}"

    #git clone https://github.com/kubler-images/goss.git "${DISTRIBUTION_DIR}"
    #cd "${DISTRIBUTION_DIR}"
    #git checkout tags/v${_goss_version}
    #echo "building goss ${_goss_version}.."
    ...
    #log_as_installed "manual install" "goss-${_goss_version}" "https://goss.org/"
